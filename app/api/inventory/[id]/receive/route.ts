...
export async function POST(
  ...
  try {
    ...
    const nextUnitCost = parsed.data.unit_cost ?? normalizeNumber(item.unit_cost ?? item.unitCost);
    ...
    const updateResult = await updateInventoryRow(supabase, companyId, itemId, {
      ...
      ...(parsed.data.to_location ? { location: parsed.data.to_location } : {}),
    });
    ...
    const txResult = await insertInventoryTransaction(supabase, {
      ...
      unit_cost: nextUnitCost,
      from_location: null,
      to_location: parsed.data.to_location ?? item.location ?? "",
    ...
    return NextResponse.json({
      item: {
        inventory: mapInventory(updateResult.data ?? item, nextUnitCost),
