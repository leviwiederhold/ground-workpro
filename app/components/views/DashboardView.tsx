...
export function DashboardView({ jobs, jobsLoading, equipment, employees, workOrders, inventory, currentRole, setCurrentView, setShowModal, ui }) {
  const { StatGrid, StatCard, Card, Button, Icon, Badge, ProgressBar, LeafletMap } = ui;
  ...
  const loadDashboardSummary = useCallback(async () => {
    ...
    try {
      ...
      const gettingStarted = item?.sections?.gettingStarted;
      if (gettingStarted) {
        setOnboardingItems(
  ...
  const activeJobsSection = dashboardSummary?.sections?.activeJobs;
  const quickActionsSection = dashboardSummary?.sections?.quickActions;
  const alertsSection = dashboardSummary?.sections?.alerts;
  const weatherSection = dashboardSummary?.sections?.weather;
  const equipmentLocationsSection = dashboardSummary?.sections?.equipmentLocations;
  const openWorkOrdersSection = dashboardSummary?.sections?.openWorkOrders;
  const showGettingStarted = dashboardSummary?.sections?.gettingStarted !== undefined;
            ...
            <Card className="p-4">
              <div className="flex items-center justify-between mb-3">
                ...
                <Button
                  variant="ghost"
                  size="sm"
                  disabled={onboardingDismissSaving}
                  onClick={() => setDismissedState(!onboardingDismissed)}
                >
                ...
                <div className="space-y-3">
                  ...
                  <div className="space-y-2">
                    {onboardingItems.map((item) => (
                      <div
                        key={item.key}
                        className="flex items-start gap-2 p-2 rounded-lg border border-gray-100"
                  ...
                  <div className="flex items-center justify-between">
                    <Button variant="ghost" size="sm" disabled={onboardingDismissSaving} onClick={() => setDismissedState(true)}>
                      Dismiss card
                    </Button>
                    {effectiveRole === 'admin' && (
                      <Button variant="ghost" size="sm" disabled={onboardingResetLoading} onClick={resetChecklist}>
                ...
                {quickActionsSection.items.map((action) => {
                  ...
                  return (
                    <Button
                      ...
                      variant="secondary"
                      size="sm"
                      className={`justify-start ${isSingleWide ? 'col-span-2' : ''}`}
              ...
              <div className="space-y-3">
                {alertsSection.items.length === 0 ? (
                  ...
                  alertsSection.items.map((alert) => (
                    <div key={alert.key} className="flex items-start gap-3 p-2 bg-yellow-50 rounded-lg">
            ...
            <Card className="p-4">
              ...
              <div className="text-center mb-4">
                <div className="flex items-center justify-center gap-4">
                  ...
                  <div>
                    <p className="text-4xl font-bold text-gray-900">{weatherSection.item.tempF}°F</p>
                    <p className="text-sm text-gray-500">{weatherSection.item.condition}</p>
                  </div>
                </div>
              </div>
              {weatherSection.item.note && (
                <div className="mt-3 p-2 bg-blue-50 rounded-lg flex items-center gap-2">
                  <Icon name="circle-info" className="text-blue-500" />
                  <span className="text-xs text-blue-800">{weatherSection.item.note}</span>
                </div>
              )}
            </Card>
          )}
