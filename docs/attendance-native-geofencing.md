...
- `src/lib/attendance/nativeGeofence.ts` — the JS side of the plugin contract:
  ...
  `getNativeGeofenceHealth()`, `onGeofenceTransition()`. All safe no-ops when the
  native plugin is absent.
- `src/lib/attendance/offlineQueue.ts` + `offlineQueueClient.ts` — a durable,
  self-flushing queue (stable event id → dedupe, preserved timestamps, retry on
  network recovery, persisted across restarts) for events generated by the
  web/foreground layer. The **native** layer keeps its own offline queue for
  transitions fired while the WebView is dead.
- `JobsiteTimeEmployeeCard` registers the assigned jobs' regions when the native
  plugin is present, enrolls a device attendance credential (PR: background
  auth) for background submission, queues foreground transitions, and stands the
  foreground watch down so events aren't duplicated.
- Native registration + last-event health flows into the attendance diagnostics
  (`getNativeGeofenceHealth()` → registered geofences + last native event).
