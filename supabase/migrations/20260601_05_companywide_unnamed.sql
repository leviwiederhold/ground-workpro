-- The permanent company-wide chat should start UNNAMED (name = null) so the
-- owner is prompted to name it on first open, instead of showing "Companywide".
-- One chat per company, participant sync, and the delete guard are unchanged.

-- 1. Recreate ensure_companywide_thread to insert an unnamed thread (name null).
...
begin
  ...
  -- name is left NULL: the owner names it on first open.
...
-- 2. Backfill: clear the old default "Companywide" name on existing chats so
--    owners get prompted to name them. (A name the owner actually chose is kept.)
update public.message_threads
set name = null
where is_companywide and name = 'Companywide';
