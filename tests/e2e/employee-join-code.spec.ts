...
test('shared employee code regenerates securely, assigns Employee, and blocks the web dashboard', async ({ page, browser }) => {
  ...
  }, { timeout: 30_000 }).toBe('active::team_member::true');
  ...
  await page.getByTestId(`employee-role-review-select-${employeeId}`).selectOption('crew_lead');
  const roleReviewRequest = page.waitForRequest((request) =>
    request.method() === 'PATCH' &&
    new URL(request.url()).pathname === `/api/team/members/${employeeId}/permissions`
  );
  await page.getByTestId(`employee-role-review-save-${employeeId}`).click();
  expect((await roleReviewRequest).postDataJSON()).toMatchObject({
    role: 'crew_lead',
    mark_reviewed: true,
  });
  await expect.poll(async () => {
  ...
  }, { timeout: 30_000 }).toBe('crew_lead::false');
  ...
  await expect.poll(async () => {
    ...
    return `${String(employee?.role ?? '')}::${String(employee?.roleReviewPending ?? '')}`;
  }, { timeout: 30_000 }).toBe('team_member::false');
