...
test('onboarding checklist persists and is role-aware', async ({ page }) => {
  ...
  await expect(page.getByTestId('onboarding-item-company_profile')).toBeVisible();
  await expect(page.getByTestId('onboarding-item-create_bid')).toBeVisible();

  const persistenceResponse = await page.request.post('/api/onboarding/checklist/complete', {
    data: { key: 'company_profile', completed: true },
  });
  ...
  if (persistenceResponse.status() === 200) {
    ...
    const companyProfile = checklistItems.find((entry: { key: string }) => entry.key === 'company_profile');
  ...
  await expect(page.getByRole('heading', { name: 'Getting Started' })).toBeVisible();
  await expect(page.getByTestId('onboarding-item-upload_attachment')).toBeVisible();
  await expect(page.getByTestId('onboarding-item-create_bid')).toHaveCount(0);
});
