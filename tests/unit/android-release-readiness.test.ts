...
test("Android release identity, API level, signing guard, and production start route are explicit", () => {
  ...
  const androidSync = read("scripts/android-sync.mjs");
  ...
  assert.match(appGradle, /task\.name == 'preReleaseBuild'/);
  ...
  assert.match(androidSync, /ground-workpro\.vercel\.app/);
  assert.match(androidSync, /server\.appStartPath/);
});

test("every Android source and test package uses the permanent application identity", () => {
  const androidText = execFileSync("git", ["grep", "-n", "com\\.", "--", "android"], {
    cwd: repoRoot,
    encoding: "utf8",
  });
  assert.doesNotMatch(androidText, /com\.getcapacitor\.(myapp|app)/);
  assert.doesNotMatch(androidText, /com\.leviwiederhold\.groundworkpro/);
});
...
test("Android manifest declares release permissions without weakening storage or backup boundaries", () => {
  ...
  assert.match(read("android/app/src/main/res/mipmap-anydpi-v26/ic_launcher_round.xml"), /<adaptive-icon/);
});
...
test("Android native health reports the OS authorization gates used by CEO readiness", () => {
  ...
  assert.match(readinessRoute, /shouldAcceptReadinessReport/);
